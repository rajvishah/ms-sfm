...
int FeatureMatcher::computeFmatrix(double* Fdata) {
    ...
    for(int i=0; i < matches.size(); i++) {
        ...
        imgPts1.push_back( cv::Point2f(srcKeysInfo[idx1].nx,srcKeysInfo[idx1].ny));
        imgPts2.push_back( cv::Point2f(refKeysInfo[idx2].nx,refKeysInfo[idx2].ny));
...
void FeatureMatcher::computeEpipolarLines() {
    ...
    for(int i=0; i < numSrcPts; i++) {
        epiLines[i].resize(3);
        double x1[] = {srcKeysInfo[i].nx, srcKeysInfo[i].ny, 1.0};
...
void FeatureMatcher::verifyEpipolarConstraints() {
    ...
    for(int f=0; f < matches.size(); f++) {
        ...
        double x2[] = {refKeysInfo[matchingPt].nx, refKeysInfo[matchingPt].ny, 1.0};
...
int FeatureMatcher:: bfMatch() {
    ...
    for(int i=0; i < pointGroups.size(); i++) {   
        ...
        for(int j=0; j < pointGroups[i].size(); j++) {
            ...
            double x2[] = {refKeysInfo[matchingPt].nx,
                refKeysInfo[matchingPt].ny, 1.0};
            double line[3];
            geometry::ComputeEpipolarLine(x2,fMatrix.data(),line,true); 

            double x1[] = {srcKeysInfo[qPtIdx].nx,
                srcKeysInfo[qPtIdx].ny, 1.0};
            float epiDist2 = 
...
void FeatureMatcher::getProbableMatches(int idx, vector<int>& probMatches) {
    ...
    for(int i=0; i < numLinePts; i++) {
        ...
        x[i] = x[i] + ((float)rWidth/2);
        y[i] = -y[i] + ((float)rHeight/2);
    }
...
bool pairCompare(const pair<unsigned long long int, int>& firstElem, const      pair<unsigned long long int, int>& secondElem) {
...
void FeatureMatcher::clusterPointsFast() {
    vector< pair<unsigned long long int, int> > clubSortedArr;
    clubSortedArr.reserve( epiLines.size()+ 500 );

    for(unsigned int i=0; i < epiLines.size(); i++) {
        ...
        x1 = (short)(endPoint1[0] + (float)(rWidth)/2.0f);
        y1 = (short)(-endPoint1[1] + (float)(rHeight)/2.0f);
        x2 = (short)(endPoint2[0]+ (float)(rWidth)/2.0f);
        y2 = (short)(-endPoint2[1]+ (float)(rHeight)/2.0f);

  //      printf("\nUnnormalized End Points : %f %f %f %f", endPoint1[0], endPoint1[1], endPoint2[0], endPoint2[1]);
//        printf("\nNormalized End Points : %f %f %f %f", x1, y1, x2, y2);


        unsigned long long int l = packLineEndPoints(x1,y1,x2,y2);
        clubSortedArr.push_back(make_pair(l,i));
    }

    std::sort(clubSortedArr.begin(), clubSortedArr.end());
    ...
    pointToLineGroupIdx.resize(numSrcPts, -1);
    for(int i= 0; i < clubSortedArr.size(); i++) {
        ...
        x1 = x1 - (short)((float)(rWidth/2.0f));
        x2 = x2 - (short)((float)(rWidth/2.0f));
        y1 = - y1 + (short)((float)(rHeight/2.0f));
        y2 = -y2 + (short)((float) (rHeight/2.0f));
