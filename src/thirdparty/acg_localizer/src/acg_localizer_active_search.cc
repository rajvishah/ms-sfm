...
int main (int argc, char **argv)
{
  ...
  if( argc >= 13 ) {
      std::cout << "Argv 12 and 13 " << argv[12] << " " << argv[13];
        std::fflush(stdout);
      singleImageLocalization = true;
    imageToLocalize = (uint32_t) atoi(argv[12]);
    consider_K_nearest_cams = (uint32_t) atoi( argv[13] );
  }
  std::fflush(stdout);
  std::string outFile;
  if(singleImageLocalization) {
      char timeFileName[1000];
      sprintf(timeFileName,"%s/timing-%04d.txt", argv[7], imageToLocalize);
      outFile=std::string(timeFileName);
  } else {
    outFile = std::string(argv[7]) + "/timings.txt";
  }
...
// Rajvi - Introduce OpenMP here? Compute time for preprocess?

  for( uint32_t i=0; i<nb_keyfiles; ++i, N+=1.0 )
  {
      if(singleImageLocalization) {
          if(i != imageToLocalize)
              continue;
        }

      char parFileName[1024];
        sprintf(parFileName,"%s/%04d-cam-par.txt", argv[7], queryKeyMap[i]);
...
//         std::cout << " "  << corr_3D_to_2D.size() << std::endl;
    ...
    if( inlier.size() >= minimal_RANSAC_solution )
    {
        ...
        for(int inl=0; inl < inlier.size(); inl++) {
            std::pair<uint32_t, uint32_t> pair = final_correspondences[inlier[inl]];
