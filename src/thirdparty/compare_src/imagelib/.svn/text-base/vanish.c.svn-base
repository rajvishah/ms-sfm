/* 
 *  Copyright (c) 2008-2009  Noah Snavely (snavely@cs.cornell.edu)
...
#define HOUGH_SAMPLES_THETA 128
#define HOUGH_SAMPLES_PHI 128
#define HOUGH_SIGMA 0.015 // 0.01 // 0.005
#define HOUGH_SIGMA_DEGREES 2.5 // 5.0
...
/* Find vanishing points using a Hough transform */
int compute_vps_hough_edges(img_t *img, int num_edges, edge_link_t *edges,
                            int num_vps, int min_support, 
                            double *vps, int *support) 
{
    // int min_support = 10;
    ...
    int num_detected = 0;
    for (round = 0; round < num_vps; round++) {
	...
	ymax = size2 * sin(theta_max) * sin(phi_max) / cos(phi_max) + h2;
        ...
        vps[2 * num_detected + 0] = xmax;
        vps[2 * num_detected + 1] = ymax;
	...
	for (i = 0; i < num_edges; i++) {
	    ...
	    if (RAD2DEG(acos(fabs(dot))) > 90.0 - HOUGH_SIGMA_DEGREES /*4.0 * HOUGH_SIGMA*/) {
		line_used[i] = 1;
		edges[i].idx = round;
                edges_removed++;
	    }
        ...
        if (edges_removed < min_support) {
            break;
        }

        support[num_detected] = edges_removed;
