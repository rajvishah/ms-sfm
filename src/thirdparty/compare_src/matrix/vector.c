...
v3_t v3_quick_median(int n, const v3_t *v) {
    int step = 1;

    if (n <= 1000)
        return v3_median(n, v); /* just compute the exact median */
...
/* Compute the average distance from each point to the mean */
double v3_average_dist_to_mean(int n, const v3_t *v)
{
    v3_t mean = v3_mean(n, v);
    double dist_sum = 0.0;
    int i;
    for (i = 0; i < n; i++) {
        dist_sum += v3_mag(v3_sub(v[i], mean));
    }
