...
__global__ void convertnInitializeSift(int source_num_keys,int target_num_keys,int *target_siftID,int *source_siftID)
{
	int siftID = blockIdx.x*blockDim.x + threadIdx.x;	
	int j;
	if(siftID < source_num_keys)
	{
		source_siftID[siftID] = siftID;
	}
	if(siftID < target_num_keys)
	{
		target_siftID[siftID] = siftID;
...
__global__ void hello()//SiftGPU::SiftKeypoint *d_keys2,float *d_target_x,float *d_target_y,int target_num_keys)
{

	int siftID = blockIdx.x;
	int tid = threadIdx.x;	
	int j;
	int dist = 0;
	if(tid == 0)
	{
		for(j = 0 ; j < 128; j++)
		{
			printf("%f ",(512*tex1Dfetch(tex_targetDescriptor,(siftID*128)+j)));
		}
		printf("\n");
	}
	__syncthreads();
...
//__global__ void computeGridID(float *x,float *y, int *gridID1,int *gridID2,int *gridID3,int *gridID4, int num_keys,int gridSize,int numXGrids1,int halfSize,int numXGrids2,int numGrids,int numGridsXOv,int numGridsYOv)
{	
...
//		double simpleX = ((double)x[siftID]/(double)gridSize);
//		double simpleY = ((double)y[siftID]/(double)gridSize);
...
//		if(gridID4[siftID] < 0 || gridID3[siftID] < 0 || gridID2[siftID] < 0 || gridID1[siftID] <= 0)
//			printf("%d %d %d %d %d %f %f %lf %lf %d %d\n",siftID,gridID1[siftID],gridID2[siftID],gridID3[siftID],gridID4[siftID],keys[siftID].x,keys[siftID].y,simpleX,simpleY,numXGrids1,numXGrids2);
		if(gridID2[siftID] < 0)
			gridID2[siftID] = 0;
		if(gridID3[siftID] < 0)
			gridID3[siftID] = 0;
		...
		//gridID2[siftID] = gridID2[siftID] + numGrids;
//		  gridID3[siftID] = gridID3[siftID] + numGrids + numGridsXOv;
//		  gridID4[siftID] = gridID4[siftID] + numGrids + numGridsXOv + numGridsYOv;
...
__global__ void findTopOfBucket(int *d_gridID,int *d_gridID2,int *d_gridID3,int *d_gridID4, int num_elements,int *d_topOfBucket_ptr,int *d_topOfBucket2_ptr,int *d_topOfBucket3_ptr,int *d_topOfBucket4_ptr,int numGrids,int numGridsXOv,int numGridsYOv,int numGridsXYOv)
{
	...
	if(tidx < num_elements && tidx != 0) 
	{
		...
		//		if(tidx == 100)
		//			printf("%d %d %d %d\n",d_gridID[0],d_gridID2[0],d_gridID3[0],d_gridID4[0]);
...
__device__ void getBestGrid(float x, float y, int gridSize,int halfSize,int numXGrids1,int numXGrids2,int numGridsXOv,int numGridsYOv,int *gridid, int *which,int numGrids)
{
	...
	idx[3] = floor(simpleX -ov)*numXGrids2 + floor(simpleY - ov);
	...
	//if(blockIdx.x == 100)
	//	printf("%f %f %f %f %f %f %f %f\n",x,y,g1Xc,g1Yc,g2Xc,g2Yc,g3Xc,g3Yc);
	...
	for(int id=0; id < 4; id++) {
		...
		//		if(blockIdx.x == 100)
		//			printf("\nGrid Id %d Dist %f", idx[id], dists[id]);
	...
	//	if(blockIdx.x == 100)
	//		printf("\nGrid Id %d Dist %f WhichGrid %d", minIdx, min_g_dist, idno);
...
__global__ void	findPotentialMatchesForEachCluster(int *gridID,int *target_siftID,int *topOfBucket,int *gridID2,int *target_siftID2,int *topOfBucket2,int *gridID3,int *target_siftID3,int *topOfBucket3,int *gridID4,int *target_siftID4,int *topOfBucket4,int gridSize,int *clusterLocation,long long int *epipolarPoints,int *numSift,int *numSift2,int *numSift3,int *numSift4,int *potentialMatches,int target_height,int numXGrids1,int halfSize,int numXGrids2,int numGridsXOv,int numGridsYOv,int numGrids,int numGridsXYOv)	
{
	...
	//	if(clusterID == 0 && threadID == 0)
	//	{
	//		for(int i = 0; i < numGridsYOv; i++)
	//			if(numSift3[i] > 20)
	//			printf("%d\n",numSift3[i]);
	//		for(int i = 0; i < numGridsXYOv; i++)
	//			if(numSift4[i] > 20)
	//			printf("%d\n",numSift4[i]);
	//      }
	...
	for(i = 0 ; i < num_of_pts_per_thread ; i++ )
	{
			...
			// the point lies in the rectangle
		{
			...
			if(top == -1)
			{
				...
				else if(which_grid == 2)// && grid_id < numGridsXOv)
				{
					//					if(grid_id > numGridsXOv)
					//						printf("%d \n",grid_id);
					totalNumsift += numSift2[grid_id];
					grids[++top] = grid_id;
					which[top] = which_grid;
					//					if(numSift2[grid_id]!=0)
					//						printf("2 : %d\n",numSift2[grid_id]);
				}
				else if(which_grid == 3)// && grid_id < numGridsYOv)
				{
					//					if(grid_id > numGridsYOv)
					//						printf("%d \n",grid_id);
					totalNumsift += numSift3[grid_id];
					grids[++top] = grid_id;
					which[top] = which_grid;
					//					if(numSift3[grid_id]>20)
					//						printf("3 : %d\n",numSift3[grid_id]);
				}
				else if(which_grid == 4)// && grid_id < numGridsXYOv)
				{
					totalNumsift += numSift4[grid_id];
					grids[++top] = grid_id;
					which[top] = which_grid;
					//					if(numSift4[grid_id]!=0)
					//						printf("4 : %d\n",numSift4[grid_id]);
				}
			}
	...
	__syncthreads();	// to make sure that all the threads have written their num of sifts in the shared memory
	...
	// There is an array of size 501 for each cluster, we will write the potential matches there
	...
	potentialMatches[501*clusterID] = shared_array[255];	// total num of potential matches
	...
	for(i=0;i<=top;i++)
	{
		if(which[i] == 1)
		{
			if(topOfBucket[grids[i]]!=-1)
			{
				...
				{
					potentialMatches[(501*clusterID)+writePosition+k] = target_siftID[j];
					k++;
				}
		...
		else if(which[i] == 2)
		{
			if(topOfBucket2[grids[i]]!=-1)
			{
				...
				{
					potentialMatches[(501*clusterID)+writePosition+k] = target_siftID2[j];
					k++;
				}
		...
		else if(which[i] == 3)
		{
			if(topOfBucket3[grids[i]]!=-1)
			{
				...
				{
					potentialMatches[(501*clusterID)+writePosition+k] = target_siftID3[j];
					k++;
				}
		...
		else if(which[i] == 4)
		{
			if(topOfBucket4[grids[i]]!=-1)
			{
				...
				{
					potentialMatches[(501*clusterID)+writePosition+k] = target_siftID4[j];
					k++;
				}
...
__global__ void	findMatches(int *matches,int *clusterID,int *potentialMatches)
{
	//========================================================================
	//
	...
	num_potential_matches = potentialMatches[501*cluster_id];
	...
	for(i = 0;i < num_pts_per_thread; i++)
	{
		...
		{
			target_siftID = potentialMatches[(501*cluster_id)+1+(num_pts_per_thread*threadID)+i];
			dist = 0;
			for( j = 0 ; j < 128 ; j++) 
			{
				dist+= ((tex1Dfetch(tex_sourceDescriptor,(source_siftID*128)+j) - (tex1Dfetch(tex_targetDescriptor,(target_siftID*128)+j)))*(tex1Dfetch(tex_sourceDescriptor,(source_siftID*128)+j) - (tex1Dfetch(tex_targetDescriptor,(target_siftID*128)+j))));
			}
			if(dist <= min)
			{
				next_min = min;
...
//				printf("%d\n",dist);
	...
	//	printf("%d %d %d %d\n",threadID,threadID+blockDim.x,threadID+2*blockDim.x,threadID+3*blockDim.x);
	shared_array[threadID] =(int) min;
	shared_array[threadID+blockDim.x] = (int)next_min;
	...
	if(threadID == 0)
	{
		...
		// Ratio test
		if((float)min1/min2 < 0.36 && min1_id > 0 && num_potential_matches > 9)	// we store the squared distances so the ratio becomes 0.36 instead of 0.6
		{
			// Declare match
...
void Matcher::matchImagePair(sift_img source,sift_img target,float *f_matrix,std::vector<SiftGPU::SiftKeypoint> &keys1,std::vector<float > &descriptors1 ,std::vector<SiftGPU::SiftKeypoint> &keys2,std::vector<float > &descriptors2 )
{
...
//	cudaMalloc((void**)&desc2, 128*sizeof(float)*target.num_keys);
	...
	cudaMemcpy(d_source_x, source.x, sizeof(float)*source.num_keys, cudaMemcpyHostToDevice); 
	cudaMemcpy(d_source_y, source.y, sizeof(float)*source.num_keys, cudaMemcpyHostToDevice); 

	cudaMemcpy(d_target_x, target.x, sizeof(float)*target.num_keys, cudaMemcpyHostToDevice); 
	cudaMemcpy(d_target_y, target.y, sizeof(float)*target.num_keys, cudaMemcpyHostToDevice); 
...
//	cudaMemcpy(desc2, descriptors2.data(), sizeof(float)*target.num_keys*128, cudaMemcpyHostToDevice); 
	clock_gettime(CLOCK_MONOTONIC, &t1);
	for(int i = 0; i < source.num_keys; i++)
	{   
		for(int j = 0; j < 128; j++)
		{
			source.keypoints[(i*128)+j] = (unsigned char)(512*descriptors1[(i*128)+j]); 
		}
	...
	{   
		for(int j = 0; j < 128; j++)
		{
			target.keypoints[(i*128)+j] = (unsigned char)(512*descriptors2[(i*128)+j]); 
		}
	...
	i = source.num_keys;
	if(i < target.num_keys)
		i = target.num_keys;
	int numBlocks = ceil(((float)i)/threadsPerBlock);

	//convertnInitializeSift<<<numBlocks, threadsPerBlock>>>(source.num_keys,target.num_keys,d_target_siftID,d_source_siftID);
	//cudaMalloc((void**)&d_source_siftID, sizeof(int)*source.num_keys);
	//cudaMalloc((void**)&d_target_siftID, sizeof(int)*target.num_keys);
...
//	thrust::host_vector<float> h_d1(descriptors1.begin(), descriptors1.end());
//	thrust::host_vector<float> h_d2(descriptors2.begin(), descriptors2.end());
//	thrust::device_vector<float> d1 = h_d1;
//	thrust::device_vector<float> d2 = h_d2; 
//	thrust::device_ptr<float> d_desc1_ptr(desc1);
//	thrust::device_ptr<float> d_desc2_ptr(desc2);
...
//	unsigned char * desc1_ptr = thrust::raw_pointer_cast(u_d1.data());
//	unsigned char * desc2_ptr = thrust::raw_pointer_cast(u_d2.data());

//	unsigned char *h = (unsigned char *)malloc(128*target.num_keys*sizeof(unsigned char));
//	thrust::host_vector<unsigned char> H = u_d2;
...
//	float *h_d = (float *)malloc(128*target.num_keys*sizeof(float));
//	cudaMemcpy(h_siftID, d_target_siftID, sizeof(int)*target.num_keys, cudaMemcpyDeviceToHost);
//	cudaMemcpy(h_d, desc2, sizeof(float)*128*target.num_keys, cudaMemcpyDeviceToHost);
//	for(i = 0 ; i < 128*target.num_keys; i++)
//	{
//		if(512*h_d[i] > 0)
//			printf("%f\n",(512*h_d[i]));
//		if(h_siftID[i] != target.siftID[i])
//			printf("%d %d %d\n",i,h_siftID[i],target.siftID[i]);
//	}

	cudaMemcpy(d_target_keypoints, target.keypoints, sizeof(unsigned char)*target.num_keys*128, cudaMemcpyHostToDevice); 
	cudaMemcpy(d_source_keypoints, source.keypoints, sizeof(unsigned char)*source.num_keys*128, cudaMemcpyHostToDevice); 

//	cudaMemcpy(d_source_siftID, source.siftID, sizeof(int)*source.num_keys, cudaMemcpyHostToDevice); 
//	cudaMemcpy(d_target_siftID, target.siftID, sizeof(int)*target.num_keys, cudaMemcpyHostToDevice); 
...
//	cudaBindTexture (0, tex_targetDescriptor, desc2_ptr, sizeof(unsigned char)*target.num_keys*128 );
//	cudaBindTexture (0, tex_sourceDescriptor, desc1_ptr, sizeof(unsigned char)*source.num_keys*128 );
...
//	int gridSize = 8;	
	//	int numGridsx = ceil(((float)target_width)/gridSize);	// num of grids in a row
	//	int numGridsy = ceil(((float)target_height)/gridSize);	// num of grids in a column
	//	int numGrids = numGridsx*numGridsy;			// total num of grids
	Grid target_grid;
	target_grid.g_init(target);
	//==============================================================
	//	int imageWidth = target_width;
	//	int imageHeight = target_height;
	//	if(gridSize%2 == 1) gridSize++;
	//	int halfSize = gridSize/2;

	//	int numXGrids1 = floor(imageWidth/gridSize);
	//	int numYGrids1 = floor(imageHeight/gridSize);
	//	int numXGrids2 = floor((imageWidth-halfSize)/gridSize);
	//	int numYGrids2 = floor((imageHeight-halfSize)/gridSize);

	//	int numGrids = numXGrids1*numYGrids1;
	//	int numGridsXOv = numXGrids1*numYGrids1;
	//	int numGridsYOv = numXGrids1*numYGrids1;
	//	int numGridsXYOv = numXGrids1*numYGrids1;
	//	int numGridsXOv = numXGrids2*numYGrids1;
	//	int numGridsYOv = numXGrids1*numYGrids2;
	//	int numGridsXYOv = numXGrids2*numYGrids2;

	//	float overlap = (float)halfSize/gridSize;


	//==============================================================

/*	for(i = 0 ; i < target.num_keys; i++)
	{
		if(target.x[i]!=keys2[i].x)
			printf("x\n");
		if(target.y[i]!=keys2[i].y)
			printf("y\n");
	}
...
//	for(i = 0; i < i)
	...
	/*computeGridID<<<numBlocks, threadsPerBlock>>>(d_target_x,d_target_y, d_gridID1, d_gridID2, d_gridID3, d_gridID4, target.num_keys,target_grid.gridSize, target_grid.numXGrids1,target_grid.halfSize,target_grid.numXGrids2,target_grid.numGrids,target_grid.numGridsXOv,target_grid.numGridsYOv);*/
...
//	cudaMemcpy(h_gridID, d_gridID3, sizeof(int)*target.num_keys, cudaMemcpyDeviceToHost);
//	int *h_siftID = (int *)malloc(target.num_keys*sizeof(int));
//	cudaMemcpy(h_siftID, d_target_siftID3, sizeof(int)*target.num_keys, cudaMemcpyDeviceToHost);
//	for(i = 0 ; i < target.num_keys; i++)
//	{
//		printf("%d %d %d\n",i,h_siftID[i],h_gridID[h_siftID[i]]);
//	}

	//	cudaMemcpy(h_gridID, d_gridID, sizeof(int)*target.num_keys, cudaMemcpyDeviceToHost);
	...
	//copy topOfBucket from device to host
	//h_topOfBucket = d_topOfBucket;

//	for(i=0;i<target.num_keys;i++)	
//		std::cout << i << ' ' << h_gridID[i] << ' ' << h_siftID[i] << std::endl;
//	 

	//	for(i=0;i<numGrids;i++)	
	//	{
	//		if(d_numSift[i]!=0)
	//			std::cout << d_numSift[i] << std::endl;
	//	}


//		for(i=0;i<target_grid.numGridsXOv;i++)	
//			if(d_numSift2[i] > 2 )
//				std::cout << d_numSift2[i] << std::endl;

//	std::cout << d_numSift2[10] << std::endl;
//		for(i=0;i<target_grid.numGridsYOv;i++)	
//			if(d_numSift3[i]>3)
//				std::cout << d_numSift3[i] << std::endl;

	//	for(i=0;i<numGridsXYOv;i++)	
	//		if(d_numSift4[i]!=0)
	//			std::cout << d_numSift4[i] << std::endl;



	//hello<<<numBlocks, threadsPerBlock>>>(d_numSift_ptr,d_numSift2_ptr,d_numSift3_ptr,d_numSift4_ptr,numGridsXOv);
	...
	cudaMalloc((void **)&d_potentialMatches,numClusters*501*sizeof(int));	// allocate memory on device
	...
	findMatches<<<numBlocks, threadsPerBlock,4*threadsPerBlock*sizeof(int)>>>(d_matches_ptr,d_clusterID,d_potentialMatches);
//	hello<<<numBlocks, threadsPerBlock>>>();
	...
	//	visualizeMatches(argv[4],argv[5],h_matches,source_x,target_x,source_y,target_y);
...
*/	free(h_cluster);
	...
	int num_matches = 0;
	for(i = 0 ; i < h_matches.size(); i++)
	{
		if(h_matches[i]!=-1)
		{
			num_matches++;
			matches.push_back(std::make_pair(i,h_matches[i]));
			//			std::cout << i << ' ' << h_matches[i] << std::endl;
		}
	}
