...
//	cudaMemcpy(d_source_keypoints, source.keypoints, sizeof(unsigned char)*source.num_keys*128, cudaMemcpyHostToDevice); 
	...
	thrust::device_vector<float> d1(descriptors1.begin(), descriptors1.end());
	thrust::device_vector<float> d2(descriptors2.begin(), descriptors2.end());
...
//	unsigned char h[128*target.num_keys];
//	thrust::host_vector<unsigned char> H = Y;
