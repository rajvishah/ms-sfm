...
int main( int argc, char* argv[]) {
    ...
    if(level==-1) {
        ...
        stringstream strm;
        strm << argv[1] << "/loc" << setw(1) << level << "/bundler_loc/";
        bundlePath = strm.str();
    }
    ...
    for(int i=0; i < nearbyImages.size(); i++) {
        ...
        for(int j=0; j < keyPtPairs.size(); j++) {
            ...
            matches3D_2D.push_back(make_pair(pt3Idx, keyIdx));
    ...
    */
    ...
    bool status = BundleRegisterImage(data, pt3, pt2, pt3_idx, pt2_idx, NULL);
    if(status) {
        ...
        printf("\nCould not register the image %d", imageIdx);
    }
